...
def setup_sidebar():
    sidebar.button("Показать задачи", on_click=update_session_tracking, args=(menu_key, TaskierMenuOption.SHOW_TASKS.value))
    sidebar.button("Новая задача", on_click=update_session_tracking, args=(menu_key, TaskierMenuOption.NEW_TASK.value))
    selected_db = sidebar.radio("Тип базы данных", [x.value for x in TaskierDBOption])
    set_db_option(selected_db)
    sidebar.button("Загрузить тестовые данные", on_click=Task.load_seed_data)
...
def setup_filters():
    ...
    with sidebar.expander("Сортировка и фильтрация", expanded=True):
        filter_params[TaskierFilterKey.SORTING_KEY.value] = st.selectbox("Сортировать по", sorting_keys)
        filter_params[TaskierFilterKey.SORTING_ORDER.value] = st.radio("Порядок сортировки", sorting_orders)
        filter_params[TaskierFilterKey.SELECTED_STATUSES.value] = st.multiselect("Задачи по статусу", options=status_options)
        filter_params[TaskierFilterKey.SELECTED_URGENCIES.value] = st.multiselect("Задачи по важности", options=range(1, 6))
        filter_params[TaskierFilterKey.SELECTED_CONTENT.value] = st.text_input("Задачи по описанию")


def setup_deletion():
    task = session[working_task_key]
    text_title = sidebar.text_input("Название задачи для удаления", key="existing_delete")
    submitted = sidebar.button("Удалить задачу")
    if submitted:
        if text_title == task.title:
            task.delete_from_db()
            sidebar.success("Ваша задача удалена.")
        else:
            sidebar.error("Введите точное название задачи для удаления.")
...
def show_tasks():
    ...
    for task in tasks:
        ...
        col1.write(str(task))
        col2.button("Подробнее", key=task.task_id, on_click=wants_task_detail, args=(task,))
...
def show_task_detail():
    ...
    submitted = form.form_submit_button("Обновить задачу")
    if submitted:
        try:
            task.update_in_db()
        except TaskierError:
            form.error("Невозможно обновить, возможно задача удалена.")
    else:
        session[working_task_key] = task
        form.success("Задача обновлена!")


def show_new_task_entry():
    with st.form("new_task_form", clear_on_submit=True):
        st.title("New Task")
        title = st.text_input("Название", key="new_task_title")
        desc = st.text_input("Описание", key="new_task_desc")
        urgency = st.slider("Уровень важности", min_value=1, max_value=5)
